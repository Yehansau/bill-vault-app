    ...
    <View className="flex-1 bg-white px-2">
      ...
      <View className="flex-col rounded-full border border-[#CAC4D0] mt-5 p-4">
        ...
        {categoryData.map((item) => (
          <CategoryWise key={item.id} category={item.category} amount={item.amount} />
        ))}
      </View>
      ...
      <View className="flex-row mt-10 justify-center items-center">
        <TouchableOpacity
          className="rounded-full bg-white border border-[#3B1E54] p-3"
        ...
        <TouchableOpacity
          className="rounded-full bg-white border border-[#3B1E54] p-3 ml-5"
